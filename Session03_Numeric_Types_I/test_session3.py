...
def test_readme_exists():
    assert os.path.isfile(
        "Session03_Numeric_Types_I/README.md"), "README.md file missing!"


def test_readme_contents():
    readme_words = [word for line in open(
        'Session03_Numeric_Types_I/README.md', 'r', encoding="utf-8") for word in line.split()]
    assert len(
...
def test_readme_proper_description():
    ...
    f = open("Session03_Numeric_Types_I/README.md", "r", encoding="utf-8")
    content = f.read()
    f.close()
...
def test_invalid_base_valueerror():
    ...
    with pytest.raises(ValueError):
        session3.encoded_from_base10(
            10, 37, '1234567890123456789012345678901234567')
...
def test_hexadecimal_conversions():
    for _ in range(50):
        ...
        assert (session3.encoded_from_base10(r_num, 16, '0123456789abcdef').lower()) == hex(
...
def test_negative_hexadecimal_conversions():
    for _ in range(50):
        ...
        assert (session3.encoded_from_base10(r_num, 16, '0123456789abcdef').lower(
        )) == '-' + hex(r_num)[3:], f"Your program returned wrong HEX conversions"
...
def test_repeating_digits_valueerror_provides_relevant_message():
    with pytest.raises(ValueError, match=r".* repeating .*"):
        session3.encoded_from_base10(
            10, 10, '012AB012ab'), 'Something is fishy! You are not using word "repeating" while talking about an error releated to "repeating" alphanumerics!!'
...
def test_float_equality_testing():
    for _ in range(10000):
        ...
        assert session3.float_equality_testing(a, b) == math.isclose(
            a, b, rel_tol=1e-12, abs_tol=1e-05), 'Aap jis number se sampark karna chahte hai, woh is samay uplabdh nahi hai, kripya thodi der baad prayas karein. The numbers you are trying to check right now are not equal, please try again later'
...
def test_manual_truncation_function():
    for _ in range(100):
        ...
        assert session3.manual_truncation_function(f_num) == math.trunc(
            f_num), 'Just because you are not able to fix this truncation error, SkyNet is going to rule the earth!'


def test_manual_rounding_function():
    for f_num in [1.25, 1.35, -1.25, -1.35]:
        assert session3.manual_rounding_function(f_num) == round(
            f_num), 'Terminator after looking at your code: I will be back! He will come back when you fix your rounding errors.'

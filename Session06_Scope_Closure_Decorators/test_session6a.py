...
def test_function_name_had_cap_letter():
    functions = inspect.getmembers(session6a, inspect.isfunction)
...
def test_function_count():
    functions = inspect.getmembers(test_session6a, inspect.isfunction)
...
def test_indentations():
    ...
    lines = inspect.getsource(session6a)
    spaces = re.findall('\n +.', lines)
    for space in spaces:
...
def test_doc_checker():
    dc = session6a.scan_fn(50)
    fib = session6a.fib
    assert dc(
        session6a.counter) == True, "Something wrong with the doc_check function."
    assert dc(fib) == True, "Something wrong with the doc_check function."


def test_doc_checker_parameter_type():
    with pytest.raises(TypeError):
        dc = session6a.scan_fn()
...
def test_func_count():
    ...
    assert session6a.func_count['add'] == 10, "Something wrong with the func_count function."
    assert session6a.func_count['mul'] == 7, "Something wrong with the func_count function."
    assert session6a.func_count['div'] == 15, "Something wrong with the func_count function."
    assert session6a.func_count['print'] == 3, "Something wrong with the func_count function."
...
def test_func_count_custom():
    ...
    dict_1 = dict()
    fc = session6a.counter_users(dict_1)
    ...
    dict_2 = dict()
    fc_new = session6a.counter_users(dict_2)
